...
**Totals, live in production since 2026-09-03:** 75 built HTML routes (the count `find dist -name '*.html' | wc -l` gives, including `404` and `thank-you`; the sitemap carries 73, correctly excluding those two) · 28 service pages · 7 Alive Pro pages · 4 blog posts · 24 videos · 9 Common Questions routes (24 answers, two hub bands) · 11 Case Studies routes (10 clients, ASSA ABLOY parked) · **92 redirects**, shipped as **184 rules** (each emitted bare and with a trailing slash). `main` and `staging` are identical as of 2026-09-07 at `a926e03`.

**Counts moved on 2026-09-07:** routes 74 → 75 and sitemap 72 → 73 with the new `/services`; redirects 93 → 92 because that page took over a URL a 302 was holding, and the redirect had to be deleted rather than repointed. Both verified on the live domain after the deploy.
...
| `/common-questions` | Common Questions hub. **Added 2026-09-01.** Had its own top-level menu row for a day; **a Resources child since 2026-09-02**, URL unchanged. Replaced `/resources/faqs` | `content/common-questions/**/*.md` |
...
92 redirects ship in `dist/_redirects` as 184 rules, generated by `scripts/postbuild.mjs` from the
tables below. The core of them are the permanent redirects from the WordPress era.
**These must ship with the Astro site.** Dropping them discards accumulated SEO
equity on the old URLs.

Audited 2026-09-02: no duplicate `from`, no rule shadowing a real built page, and
every target resolves. Two deliberate chains remain, `/precision-impact-sprints`
and `/sprints`, which 301 to `/alive-pro/precision-impact-sprints` and then 302 to
`/alive-pro`. The 302 is the marker that Precision Impact Sprints is unpublished
and coming back; flattening those two to point at `/alive-pro` would make them
permanent redirects to the wrong destination, and browsers cache a 301 hard.

In Astro, set them in `astro.config.mjs` under `redirects`, or at the host.

### Two redirects live in Cloudflare, NOT in this file

**Added 2026-09-06.** These two are not generated by `postbuild.mjs` and will never
appear in `dist/_redirects`, because `_redirects` matches **paths only** and cannot
match a hostname or a scheme. Grepping this repo for them finds nothing.

| From | To | Where it actually lives |
|---|---|---|
| `www.aliveprostudios.com/*` | `https://aliveprostudios.com/*` (301) | Cloudflare Redirect Rule, zone `d0b29c11eaf5c4f0481f9315d4ee40d8` |
| `http://` any host | `https://` (301) | Cloudflare SSL/TLS → Edge Certificates → **Always Use HTTPS** |

The www rule matches `http.host eq "www.aliveprostudios.com"`, targets
`concat("https://aliveprostudios.com", http.request.uri.path)` at 301, and preserves
the query string. It fires at the edge **before** the origin fetch, which is the only
reason it works: the `www` DNS record is proxied but was never bound to the Worker,
so anything that reaches origin returns 522.

**The `www` DNS record must stay, and must stay proxied (orange cloud).** Being
proxied is what puts requests in front of the rule. Delete it and `www` stops
resolving entirely.

Verified 2026-09-06 across all four hostname and protocol variants, deep paths and
query strings. `www/brand-voice` chains through to `/foundation/brand-voice`, so the
hostname rule **composes with** the rules below rather than shadowing them.
...
### Reported as 404 by Search Console, added 2026-09-06

Google was crawling these and getting nothing. Five have an obvious destination;
they were simply never in the table. Found by reading the "Not found (404)"
report rather than by guessing, and each destination was checked to exist in the
build before being written here.

| From | To | Code |
|---|---|---|
| `/partnership` | `/alive-pro/partnership` | 301 |
| `/why-alive-pro` | `/alive-pro/why-alive-pro` | 301 |
| `/videos` | `/work/videos` | 301 |
...
`/alive-pro/our-system` owns (decision 11).
...
- `homepage.md` exported as structured field dumps rather than prose, because the
  old homepage was assembled from page-builder blocks. It was **deleted on
  2026-09-01** along with `home-intro.md` when the new home page shipped; the
  copy now lives as real prose in `content/home/`.
