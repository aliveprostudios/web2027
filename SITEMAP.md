...
**Totals, live in production since 2026-09-03:** 74 built HTML routes (the count `find dist -name '*.html' | wc -l` gives, including `404` and `thank-you`; the sitemap carries 72, correctly excluding those two) · 28 service pages · 7 Alive Pro pages · 4 blog posts · 24 videos · 9 Common Questions routes (24 answers, two hub bands) · 11 Case Studies routes (10 clients, ASSA ABLOY parked) · **93 redirects**, shipped as **186 rules** (each emitted bare and with a trailing slash). `main` and `staging` are identical as of 2026-09-06.
...
93 redirects ship in `dist/_redirects` as 186 rules, generated by `scripts/postbuild.mjs` from the
