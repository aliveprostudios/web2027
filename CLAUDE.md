...
| ~~**`content/faqs.md` still says "four pillars"**~~ **FIXED 2026-09-01, and FAQ design B is on staging at `/faqs-b`** | Copy now says four domains and 29 years, and the eight questions are regrouped under two `##` headings, About Alive ProStudios (5) and Working Together (3); the "Precision Impact Sprints" answer is still there although Sprints is unpublished, Javad's call. Javad rejected the parked accordion's formatting (200-weight questions, tall empty rows, glyph at the page edge, no motion, no H2). `src/pages/faqs-b.astro` is the redesign: each group is a section with an eyebrow and a real H2, rows use the pillar grid with a mono index and a 600-weight question, the glyph sits beside the question, the open question turns orange with the dot, and open/close ANIMATE through a one-row grid easing 0fr to 1fr, `<details>` kept for semantics with a script managing `open` around the transition. **Trap:** a no-JS fallback rule that showed any `[open]` panel at full height silently killed the animation, because `open` is set two frames before the animating attribute; the fallback is now scoped to lists the script never marked. `/faqs-b` is noindex and out of the sitemap. When approved, its body replaces `_resources/faqs.astro` and the section ships per the steps below. `/resources/faqs` is still parked in `src/pages/_resources/faqs.astro`. To ship it: fix the copy, move the route to `src/pages/resources/`, restore the `01` number line above `## FAQs` in `content/landing/resources.md` so the menu row and the landing card come back on their own, and repoint the `/faqs` redirect in `SITEMAP.md` from `/` to `/resources/faqs` in the SAME commit |
...
| **Case Studies built 2026-09-01, ON STAGING ONLY** | 11 clients, 12 new routes (`/work/case-studies` plus one per client), and its OWN top-level menu row placed after Work at Javad's direction, despite the routes sitting under `/work`. Production is still 54 routes; staging is 66. Copy was written from Javad's per-client drafts, reviewed as a set so no two pages repeat an argument or a System entry point, and lengths run 651 to 968 words by what each client's material earns. **Only Alfred has imagery.** The other ten render correctly without it: no `hero.jpg` falls back to `VideoHero`, absent `gallery-*` renders no gallery section, absent `body-*` is simply prose. Drop files into `content/assets/case-studies/<slug>/` named `hero`, `body-NN`, `gallery-NN` and they appear with no code change. Design approved by Javad 2026-09-01 |
| **Home-B built 2026-09-01, ON STAGING ONLY** | The draft home page at `/home-b`: `noindex`, filtered out of the sitemap in `astro.config.mjs`, linked from nowhere. Sequence is problem, help, trust, method, proof, person, ask: hero with a credit line, a visitor-facing "We Are Here to Help You Grow" block with a text link to Contact, the four cards trimmed, the reel with a caption, the System rows unchanged, THREE case study tiles (new), the brand marketing reel `youtu.be/YoudIrML8lU` as a click-to-play `InlineVideo` under "Watch the Reel" (new, Javad's "important video"), a six-image gallery driven by `content/assets/home-b/gallery/` through the same `Lightbox` contract as the case studies (new, seeded from the portfolio, Javad will swap the set), the founder quote with the Alive Pro mark and the Javad Ahmadi mark linking to javad.ca (filled), the feature image credited to Alfred, a closing statement (filled) and Next Step. The second looping video and the empty Why It Matters slot are gone. The Alive Pro mark also sits beside the help statement, and the System heading breaks deliberately after "Brand-to-Revenue". Every mark ships as two files, coloured and white, toggled on `body[data-theme='dark']`; the Javad SVGs were copied from `content/assets/logos/` into `public/assets/` under clean names. **`NextStep.astro` changed SITE-WIDE on 2026-09-01**, not just here: email and city line removed, the phone large and orange, both marks on the right. **`scripts/fetch-video-meta.mjs` now also scans `hero-videos.md`, `content/home-b/page.md` and `index.astro`**, because it rewrites video-meta.json wholesale from `videos.md` and its first run for Home-B silently dropped the live home page's pinned Vimeo loop, which lives only in code. All copy lives in `content/home-b/` (`page.md` for the hero, credits, case study picks, quote and closing; `intro.md` for the help block and cards) through the `homeB` collection, so it is editable without opening `src/pages/home-b.astro`. Copy is Claude's first draft at Javad's direction, not approved. The hero is Javad's DNA-helix image (`content/assets/home-b/hero.jpg`, converted from his Desktop `hero.png`, 2026-09-01); the credit line was dropped with it, though the `heroCredit` field still works if one is wanted. The gallery holds nine. **Client logo lanes (2b), design B**: every PNG in `content/assets/logos-clients/` (32, white cards with the border baked into the file) is dealt alternately into two full-bleed lanes that glide in opposite directions on a SCROLL timeline (`view-timeline-name` on the section, lanes subscribe by name), so they move only when the visitor scrolls. Design A, a split-flap flipping wall, was rejected by Javad 2026-09-01. **Trap re-hit here:** the section is `overflow: hidden`, so a bare `view()` on a lane parked the animation at one fixed transform on every scroll position; the named timeline on the section is the fix, same as `.feature` on index.astro. Browsers without scroll timelines get a wrapped static grid. Heading is `clientsHeading` in `page.md`, currently "Our Clients". When approved: move the content into `index.astro`, delete the route and the sitemap filter, no redirect needed |
