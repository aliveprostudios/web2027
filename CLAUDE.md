...
**2026-09-06 in one paragraph.** A Codex audit dated 2026-09-05 was reviewed and
acted on. Two Cloudflare zone fixes (below), six repo fixes from the audit's
Tier 1, and then the finding that mattered most: **every redirect 404'd when the
URL ended in a slash**, which is the form WordPress published and Google indexed.
93 redirects now ship as 186 rules, verified 93 of 93 in both forms against
...
found by the 2026-09-05 Codex audit (findings S1 and S2) and fixed the next day with
two Cloudflare zone changes: a Redirect Rule sending `www` to the apex at 301 with
path and query preserved, and Always Use HTTPS. Verified live across all four
hostname and protocol variants, deep paths, query strings, and one legacy WordPress
...
**`ANALYTICS.md` is the source of truth for GA4, Search Console, Tag Manager,
Google Ads, Meta and LinkedIn.** Every account ID, what is verified working, what
is broken with evidence, what was changed and when, and a section to read first
before starting Google Ads work in October 2026.

The headline, so it is not missed: **pageviews are captured, conversions are
not.** Three Google Ads conversion tags and the GA4 conversion have never fired
on this site, because their triggers match `/thank-you/` with a trailing slash
and Astro serves `/thank-you` without one. Same class of bug as the redirect
trailing-slash trap below. Do not run paid campaigns before that is fixed.
...
- **`Astro.url.pathname` carries the `.html` extension at build time.** This site
  ...
  Comparing it to `'/contact'` silently never matches, with no error and no build
  warning: the first attempt at the `NextStep` self-link fix (2026-09-06) looked
  correct, type-checked, built clean, and changed nothing. Measured with a
  temporary `data-` attribute rather than guessed, because expressions inside HTML
  comments are not interpolated either, so a probe in a comment prints its own
  source. Strip `.html` and a trailing `/index` before comparing a pathname to a
  route, never the slash alone.
- **`www` was orphaned by the migration and served 522 to every visitor.** The apex
  was bound as a Worker Custom Domain; `www` was left behind as a proxied DNS record
  still aimed at the dead WordPress origin, so every `www` request reached
  Cloudflare's edge and then timed out at an origin that no longer exists. **DNS
  looked perfect throughout**: `www` resolves to the same anycast IP as the apex,
  which is what makes this hard to spot. The tell is in the error page's own headers,
  `x-frame-options: SAMEORIGIN` where this site sends `DENY`, proving the response
  never touched the Worker. Fixed 2026-09-06 with a Cloudflare Redirect Rule and
  Always Use HTTPS; both are documented in `SITEMAP.md`. Two lessons worth keeping:
  **zone-level configuration is invisible to this repo**, which is why nobody noticed
...
| ~~**CTA heading is white on orange at 2.92:1**~~ **ACCEPTED TRADE** | Found 2026-09-02 while verifying the button fix, pre-existing. `.cta__light` / `.cta__bold` in `BookConsult.astro`, on **70 of 74 pages**. Large text needs 3:1. Black on orange is 7.2:1 and is already the house treatment for that exact pairing on every button, so the fix is a one-line colour change. `.cqh__askLink` on the Common Questions hub is the same pairing at 13px, where the bar is 4.5:1 **CLOSED 2026-09-06 by Javad's decision: "My Design System C is final."** The contrast trades in C are deliberate and approved, and this finding is recorded as a known, accepted trade rather than an open defect. Do not reopen it, do not "fix" it, and do not re-raise it in a future audit as though it were undiscovered. Dark mode is unaffected and stays compliant. See decision 5 and the Design System C row above. |
| ~~**Related Services "Explore Now" is 3.62:1**~~ **ACCEPTED TRADE** | Found 2026-09-02, pre-existing, on **51 pages**. `.related__explore` uses the theme-aware `--orange-ink` on a permanently dark `#111111` band, so in light theme the token resolves to its dark-ground-hostile value. Needs 4.5:1 at 16px. `--brand-orange` gives 6.48:1 and is what the 14px inner span already uses. Worth checking other always-dark sections for the same pattern **CLOSED 2026-09-06 by Javad's decision: "My Design System C is final."** The contrast trades in C are deliberate and approved, and this finding is recorded as a known, accepted trade rather than an open defect. Do not reopen it, do not "fix" it, and do not re-raise it in a future audit as though it were undiscovered. Dark mode is unaffected and stays compliant. See decision 5 and the Design System C row above. |
...
| ~~**Mobile menu does not trap Tab**~~ **RESOLVED 2026-09-06** | Everything else about it is right, and verified after a 1500px scroll: body-level fixed overlay, scroll lock on `<html>`, Escape closes, focus moves in on open and returns to the burger on close, `pageshow` clears a stranded lock. What is missing is a Tab loop, so tabbing past the last item walks into content behind the overlay. Fixed 2026-09-06 with a Tab cycle in `SiteMenu.astro`, not `inert`: the overlay AND the skip link both sit outside `.page-root`, so marking that subtree inert would have contained neither. Zero-size elements are excluded from the cycle, which keeps the collapsed accordion panels out of it |
| **`VideoObject` omits `description`** | 24 nodes on `/work/videos`. `description` is recommended by Google and missing on all of them; writing 24 is content work, not a code fix. **`uploadDate` was fixed 2026-09-06** (audit finding E2): all 24 previously asserted a hardcoded `2026-01-01`, and `scripts/fetch-video-meta.mjs` was already fetching Vimeo's real `upload_date` and discarding it. Now 17 carry true dates (2019-05-16 to 2026-07-16; Bellini is 2020-10-07) and the **7 YouTube entries omit the property**, because YouTube's oEmbed carries no date and a wrong date is a false claim where a missing one only costs the rich result. Supplying those 7 needs the YouTube Data API or manual entry |
...
| **Analytics measures almost nothing usable** | **Full detail is in `ANALYTICS.md`; read that before touching GA4, Search Console, Tag Manager or Google Ads.** Measured 2026-09-06 in GA4, Search Console and GTM directly, not from code. **Conversion tracking now works**: key events `Form_Thank_You`, `close_convert_lead` and `qualify_lead` all existed but every one read "No stream data detected", so the count was zero. A `Form_Thank_You` event on `page_view` where the URL contains `/thank-you` now feeds the existing key event; verified firing in Realtime, and one test conversion in the data is Claude's. **Four gaps remain, all in `TASKS.md`:** staging serves the IDENTICAL container `GTM-PJLQRZC`, so internal traffic is in GA4, Google Ads, Meta and LinkedIn; the site pushes NOTHING to the dataLayer, so a lead carries no context about which page produced it; `tel:` and `mailto:` clicks are untracked on all 74 pages, which GA4 enhanced measurement does not cover; and a `/thank-you` refresh counts again. **Two things poison the numbers today:** the 404 page is the site's most-viewed page (163 views in 7 days, more than the homepage at 110), and traffic is mostly bots (Singapore 148 and Vietnam 49 of 297 users, against Canada 26). **Search Console** is a URL-prefix property, verified via GA or GTM rather than DNS, so it does not cover `www`; a Domain property would consolidate now that www resolves. 55 indexed, 133 not. The stale WordPress sitemap cannot be removed (GSC no longer offers it) and does not need to be: `/sitemap_index.xml` 301s to the current sitemap. **GTM** has two extra containers on the account, `GTM-KRVGH63H` (server) and `GTM-P2NFJT` (old www); both appear 0 times on the site and no server-side tagging endpoint is configured, so both are provably inert. Deleting them is Javad's call and was deliberately not done |
| **Menu marks the four pillars, 2026-09-07** | Javad's request: Foundation, Execution, Growth and Infrastructure were blending into Work, Case Studies, Alive Pro, Resources and Contact, and they are the service architecture. They now render at `font-weight: 700` against the other rows' 200, each with a small mono "Pillar N of 4" tag beside the label. **URLs are untouched**; this is display only. The flag comes from an optional `pillar` field on `NavItem` derived from `SECTIONS`, never a hard-coded name list, so a fifth pillar would number and total itself with no code change (rule 1). Label size went 22px to 18px and the number-to-label gap 16px to 10px to buy the width. **This is a deliberate departure from Design System C**, made at Javad's direction on 2026-09-07 with the design-system question explicitly waived: "These are improvements I have to make." Do not revert it as drift. One known behaviour: at 320px INFRASTRUCTURE wraps its tag to a second line, which is the designed fallback rather than a defect. Measured, not guessed: at 19px two rows wrapped, at 18px one does, and 17px does not rescue it because fourteen tracked uppercase characters are wider than the column. Nothing overflows at any width |
